...
import { MusicPlayer, primeRemoteAudioPlayback } from "./components/MusicPlayer";
...
type NetworkMessage =
  ...
  | { type: "PLAY_AUDIO"; songId: string; requestId: number }
...
export default function App() {
  ...
  const [remoteAudioRequest, setRemoteAudioRequest] = useState<{ songId: string; requestId: number } | null>(null);
  ...
  const connectGuest = useCallback((code: string, id: string) => {
    ...
    peer.on("open", () => {
      ...
      connection.on("data", (raw) => { const message = raw as NetworkMessage; if (message.type === "STATE") { setRemoteState(message.state); setPendingAction(false); } if (message.type === "IDENTITY") setPlayerId(message.playerId); if (message.type === "LOBBY") setLobbyPlayers(message.players); if (message.type === "PLAY_AUDIO" && typeof message.songId === "string" && typeof message.requestId === "number") setRemoteAudioRequest({ songId: message.songId, requestId: message.requestId }); });
  ...
  const toggleReady = () => { if (role !== "guest" || !me || !hostConnectionRef.current?.open) return; const ready = !me.ready; if (ready) void primeRemoteAudioPlayback(); setLobbyPlayers((players) => players.map((player) => player.id === playerId ? { ...player, ready } : player)); hostConnectionRef.current.send({ type: "READY", ready } satisfies NetworkMessage); };
  const playOnCurrentPlayer = useCallback(() => {
    const song = stateRef.current.currentSong;
    const targetId = stateRef.current.players[stateRef.current.currentPlayerIndex]?.id;
    if (!song || !targetId || targetId === "host") return;
    const connection = guestConnectionsRef.current.find((item) => item.open && connectionPlayerIdsRef.current.get(item) === targetId);
    connection?.send({ type: "PLAY_AUDIO", songId: song.id, requestId: Date.now() } satisfies NetworkMessage);
  }, []);
  const canPlayOnCurrentPlayer = role === "host" && status === "connected" && currentTurnPlayerId !== "host" && currentPlayerConnected && state.contentMode === "music" && state.phase === "listening";
