...
function checkWinner(players: Player[]): Player | null {
  for (const p of players) {
    if (p.timeline.length >= CARDS_TO_WIN) return p;
  }
  return null;
}
...
function reducer(state: GameState, action: Action): GameState {
  switch (action.type) {
    case "START_GAME": {
      const pool = getSongsForCategory(action.category);
      const deck = shuffleDeck(pool);
      ...
      return {
        ...
        ...startPlayerTurn(players, startIdx, deck, action.useTokens),
    ...
    case "CONTINUE_ROUND": {
      ...
      if (!drawn.song) {
        const players = state.players.map((p, i) =>
          i === state.currentPlayerIndex
            ? { ...p, timeline: [...state.workingTimeline] }
            : p
        );
        ...
        return {
          ...
          winnerId: winner.id,
          deck: [],
          message: `Korten är slut! ${winner.name} vinner!`,
        };
      }
      ...
      return {
        ...
        lastResult: null,
        revealedSong: null,
        message: `${state.players[state.currentPlayerIndex].name} fortsätter! ${state.roundCards.length} kort i potten.`,
      };
    }
      ...
      if (state.lastResult === "correct") {
        ...
        players = state.players.map((p, i) =>
          i === state.currentPlayerIndex
            ? {
                ...p,
                timeline: [...state.workingTimeline],
                tokens: state.useTokens
                  ? Math.min(MAX_TOKENS, p.tokens + (state.streak >= 3 ? 1 : 0))
                  : p.tokens,
              }
            : p
        );
        ...
        if (winner) {
          ...
          return {
            ...
            phase: "game-over",
            winnerId: winner.id,
            message: `🎉 ${winner.name} bankade till ${winner.timeline.length} hits och vinner!`,
            roundCards: [],
          };
        }
      }
      ...
      return {
        ...
        message:
          state.lastResult === "correct"
            ? `${player.name} bankade ${state.roundCards.length} kort! ${turn.message ?? ""}`
            : `${player.name}s runda slut. ${turn.message ?? ""}`,
      } as GameState;
    }
    ...
    case "SKIP_SONG": {
      ...
      if (state.deck.length === 0) {
        return { ...state, message: "Inga fler kort att byta till!" };
      }
      const players = state.players.map((p, i) =>
        i === state.currentPlayerIndex ? { ...p, tokens: p.tokens - 1 } : p
      );
      const drawn = drawSong(state.deck);
      return {
        ...state,
        ...
        deck: drawn.deck,
        placementSlot: null,
        message: `${player.name} hoppade över låten (1 token).`,
      };
    }

    case "REDRAW_AUDIO_FAIL": {
      // Free redraw when audio can't be loaded – does not cost tokens
      if (state.phase !== "listening") return state;
      if (state.deck.length === 0) {
        return { ...state, message: "Inga fler kort i leken att byta till!" };
      }
      const drawn = drawSong(state.deck);
      const player = state.players[state.currentPlayerIndex];
      return {
        ...
        deck: drawn.deck,
        placementSlot: null,
        message: `Ljudfel – nytt kort till ${player.name} (gratis).`,
      };
    }
...
export function useGame() {
  ...
  const startGame = useCallback(
    (names: string[], useTokens: boolean, category: SongCategory) => {
      dispatch({ type: "START_GAME", names, useTokens, category });
    },
    []
  );

  const placeCard = useCallback((slotIndex: number) => {
    dispatch({ type: "PLACE_CARD", slotIndex });
  }, []);
  ...
  const bankAndEnd = useCallback(() => {
    dispatch({ type: "BANK_AND_END" });
  }, []);
  ...
  const redrawAudioFail = useCallback(() => {
    dispatch({ type: "REDRAW_AUDIO_FAIL" });
  }, []);
